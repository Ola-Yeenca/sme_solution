"""Restaurant data fetcher implementation."""
...
from smeanalytica.shared.tripadvisor_adapter import TripAdvisorAdapter
from smeanalytica.config.data_sources import DataSourceConfig, DataSourcePriority
...
@dataclass
class DataPriority:
    """Priority configuration for different data types."""
    BUSINESS_INFO = [DataSource.GOOGLE, DataSource.YELP, DataSource.TRIPADVISOR]
    REVIEWS = [DataSource.TRIPADVISOR, DataSource.GOOGLE, DataSource.YELP]
    PHOTOS = [DataSource.GOOGLE, DataSource.TRIPADVISOR, DataSource.YELP]
    MENU = [DataSource.TRIPADVISOR, DataSource.YELP]
...
class RestaurantDataFetcher(BusinessDataFetcher):
    ...
    def __init__(self, business_type: BusinessType = BusinessType.RESTAURANT):
        ...
        # Initialize adapters
        self.adapters = {
            DataSource.GOOGLE: GooglePlacesAdapter(),
            DataSource.YELP: YelpAdapter(),
            DataSource.TRIPADVISOR: TripAdvisorAdapter()
        }
    ...
    async def get_business_data(self, business_name: str) -> Dict[str, Any]:
        """Get comprehensive business data from all available sources."""
        try:
            with self.performance.measure("get_business_data") as metrics:
                # Check cache first
                cache_key = f"{business_name}_{self.business_type}"
                cached_data = await self.cache.get(cache_key)
                if cached_data:
                    logger.info(f"Cache hit for {business_name}")
                    metrics.custom_metrics["cache_hit"] = True
                    return cached_data
                
                metrics.custom_metrics["cache_hit"] = False
                ...
                # Process results
                valid_results = {}
                for source, result in zip(self.adapters.keys(), results):
                    if isinstance(result, Exception):
                        logger.error(f"Error fetching from {source}: {str(result)}")
                        metrics.errors += 1
                        continue
                        
                    if validate_api_response(result, {'require_https': True}):
                        valid_results[source] = result
                        metrics.api_calls += 1
                
                # Merge data with priority
                merged_data = await self._merge_data_with_priority(valid_results)
                
                # Cache the merged data
                await self.cache.set(cache_key, merged_data, self.cache_duration)
        ...
        *args,
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Attempt {attempt + 1} failed for {source}: {str(e)}. "
    ...
    async def _merge_data_with_priority(
        self,
        results: Dict[DataSource, Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Merge data from different sources based on priority."""
        merged = {}
        
        # Merge business info
        for source in DataPriority.BUSINESS_INFO:
            if source in results:
                self._update_dict(merged, results[source].get('business_info', {}))
        
        # Merge reviews
        merged['reviews'] = []
        for source in DataPriority.REVIEWS:
            if source in results:
                merged['reviews'].extend(results[source].get('reviews', []))
        
        # Merge photos
        merged['photos'] = []
        for source in DataPriority.PHOTOS:
            if source in results:
                merged['photos'].extend(results[source].get('photos', []))
        
        # Merge menu data
        merged['menu'] = {}
        for source in DataPriority.MENU:
            if source in results:
                self._update_dict(merged['menu'], results[source].get('menu', {}))
        
        return merged
    
    def _update_dict(self, target: Dict, source: Dict) -> None:
        """Update dictionary with non-None values."""
    ...
    async def get_business_details(self, business_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            business_data = await self.get_business_data_from_google(business_name)
            if business_data:
                return business_data
                
            # Fallback to TripAdvisor if Google Places fails
            try:
                ta_data = await self._get_tripadvisor_data(business_name)
                if ta_data:
                    return ta_data
            except Exception as e:
    ...
    async def get_business_details(self, business_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            reviews = await self.get_reviews(business_name, limit=50)
            
            # Get historical data for trends
            historical_data = await self.get_historical_data(business_name)
            ...
            enhanced_data = {
                **business_data,
                ...
                'local_market_share': self._calculate_market_share(business_data, await self.get_competitors(business_name))
            }
    ...
    async def get_competitors(self, business_name: str) -> List[Dict[str, Any]]:
        ...
        # Try Google Places first
        try:
            business_data = await self.google_places.search_business(business_name)
            if business_data:
                google_competitors = await self._get_google_places_competitors(business_data)
    ...
    async def get_business_data_from_google(self, business_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            business_data = {
                ...
                "location": {
                    "lat": place.get("location", {}).get("latitude", 39.4699),
                    "lng": place.get("location", {}).get("longitude", -0.3763)
                }
            }
    ...
    async def _get_tripadvisor_data(self, business_name: str) -> Dict[str, Any]:
    ...
    async def _get_tripadvisor_competitors(self, business_name: str) -> List[Dict[str, Any]]:
    ...
    async def _get_tripadvisor_reviews(self, business_name: str) -> List[Dict[str, Any]]:
        ...
        reviews_response = await self._make_api_request(reviews_url, {})
    ...
    async def _get_google_places_competitors(self, business_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def get_historical_data(self, business_name: str, months: int = 12) -> Dict[str, Any]:
        """Get historical sales and performance data."""
        try:
            # Get business details first
            business_data = await self.get_business_details(business_name)
