...
from smeanalytica.config.ai_model_config import AIModel, ModelConfig
...
def categorize_reviews(reviews):
    ...
    for review in reviews:
        ...
        if rating >= 4:
            categories.setdefault('positive', []).append(review)
        elif rating <= 2:
            categories.setdefault('negative', []).append(review)
        else:
            categories.setdefault('neutral', []).append(review)
            
        review_text = review.get('text', '')
        if len(review_text) > 200:
            categories.setdefault('detailed', []).append(review)
            
        if review.get('is_recent', False):
            categories.setdefault('recent', []).append(review)
...
def run_sentiment_analysis(reviews):
    ...
    print(f"Positive Reviews: {len(categorized_reviews.get('positive', []))}")
    print(f"Neutral Reviews: {len(categorized_reviews.get('neutral', []))}")
    print(f"Negative Reviews: {len(categorized_reviews.get('negative', []))}")
    print(f"Recent Reviews: {len(categorized_reviews.get('recent', []))}")
    print(f"Detailed Reviews: {len(categorized_reviews.get('detailed', []))}")
    ...
    if categorized_reviews.get('recent'):
